...
def students_overview(
    ...
    db.execute(
        text("""
            ...
            LEFT JOIN tc_matches tm ON tm.sr_number = a.sr
            LEFT JOIN transfer_certificates tc ON tc.doc_id = tm.tc_doc_id

            LEFT JOIN marksheet_matches mm ON mm.sr_number = a.sr
            LEFT JOIN marksheets ms ON ms.doc_id = mm.marksheet_doc_id

            LEFT JOIN birth_certificate_matches bcm ON bcm.sr_number = a.sr
            LEFT JOIN birth_certificates bc ON bc.doc_id = bcm.bc_doc_id

            WHERE (
                :search IS NULL
                OR a.student_name ILIKE '%' || :search || '%'
                OR a.sr::text ILIKE '%' || :search || '%'
            )

            ORDER BY a.created_at DESC, a.sr DESC
            LIMIT :limit OFFSET :offset
        """),
        {
            "search": search,
