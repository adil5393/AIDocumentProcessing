...
def run_marksheet_lookup(db, doc_id: int):
    ...
    for r in rows:
        ...
        signals["student_name_score"] = name_similarity(
            ms_student_tokens, adm_student_tokens
        )

        signals["father_name_score"] = name_similarity(
            ms_father_tokens, adm_father_tokens
        )

        signals["mother_name_score"] = name_similarity(
            ms_mother_tokens, adm_mother_tokens
        )

        signals["dob_match"] = (
            ms_dob and r.date_of_birth and ms_dob == r.date_of_birth
        )
    ...
    for c in candidates.values():
        db.execute(
            text("""
                INSERT INTO marksheet_candidates
